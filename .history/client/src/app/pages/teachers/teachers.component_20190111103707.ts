...
@Component({
  ...
  providers: [TeachersDataService,Md5]   // register service to use service data or call service functions
})
  ...
  getSingleTeacherData(id){
    ...
    this._teachersDataService.getSingleTeacherF(id)
      .subscribe(result=>  {
        ...
        this.password      = Md5.hashStr(singleTeacher.password);
...
onSubmitUpdateTeacher(submitEvent){
    let data = submitEvent.value;
    let id =   data.teacher_id;
  ...
  this._teachersDataService.updateTeacherF(updateTeacherData , id)
  .subscribe(result => {
    this.teachersDataList.push(result);
    console.log(this.teachersDataList);
    // load data from db after add
    this._teachersDataService.getTeachersF()
    .subscribe(result =>
                this.teachersDataList = result);
