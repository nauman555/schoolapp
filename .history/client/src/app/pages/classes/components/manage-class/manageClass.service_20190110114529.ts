...
export class ClassDataService {
  ...
  classExistCheckF(c_name){
    ...
    //********************* Call API to get class id and name for dropdown option ******************* */

    getClassNameIdF(c_name){
      return this.http.get('http://localhost:3000/get-classNameId/')
      .map(res => res.json());
...
//*********************** Call  API for Updating Class Data *****************************
  ...
  deleteClassF(deleteStatus ,id){
    ...
    return this.http.put('http://localhost:3000/delete-class/'+id , deleteStatus ,{headers:this.headers} )
      .map(response => response.json());
