...
export class ClassDataService {
  ...
  //*********************** Call API for retreiewing single Class Data By Name ********************

  getSingleClassByNameF(id){
    return this.http.get('http://localhost:3000/get-classByName/'+id)
    .map(res => res.json());
   ...
   getSingleClassByIdF(id){
    return this.http.get('http://localhost:3000/get-classNyId/'+id)
    .map(res => res.json());
  ...
  addClassF(classDetails)
  {
    ...
    return this.http.post('http://localhost:3000/add-teacher', classDetails,{headers:this.headers})
      .map(response => response.json());
...
//*********************** Call  API for Updating Class Data *****************************
  ...
  deleteClassF(id){
    let status = 0;
    return this.http.put('http://localhost:3000/delete-teacher/'+id , status ,{headers:this.headers} )
      .map(response => response.json());
