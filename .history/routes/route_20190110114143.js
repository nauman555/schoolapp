...
//********************************************************************* */
// ******************** get Class Name and ID for dropdown *******************/
//***********************************************************************/

router.get('/get-classNameId',(req , res)=>{
    let sql = 'Select class_id , class_name  from tbl_class where status=1 ';
    let query = con.query(sql, (err, results)=>{
        if(err){
            res.json({msg: 'fail to get class'+err});
        }
        else{
            res.json(results);
        }
