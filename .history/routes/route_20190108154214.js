...
router.post('/update-teacher/:id',(req , res ,next)=>{
    ...
    let sql = 'Update tbl_teachers SET ? where teacher_id = :teacher_id';
    let query = con.query(sql,teacherDetails , :t , (err, result)=>{
        if(err){
            res.json({msg: 'fail to add contact'+err});
        }else{
            res.json({msg: 'contact added successfully'});
