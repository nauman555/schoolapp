...
router.get('/get-section/:class_id',(req , res)=>{
    let sql = 'select * from tbl_section s inner join tbl_class c on s.class_id = c.class_id INNER JOIN tbl_teachers t on s.teacher_id = t.teacher_id '
...
router.get('/get-singleSection/:section_id/:class_id',(req , res)=>{
    let sql = `select * from tbl_section s inner join tbl_class c on s.class_id = c.class_id 
                INNER JOIN tbl_teachers t on s.teacher_id = t.teacher_id 
                Where s.class_id = ${req.params.class_id} AND s.section_id = ${req.params.section_id} AND s.status=1`;
    let query = con.query(sql, (err, result)=>{
        if(err){
            res.json({msg: 'fail to get class data'+err});
        }
...
router.get('/check-sectionExists/:class_id/:section_name',(req , res)=>{
    ...
    let sql = `Select COUNT(*) AS section_present from tbl_section Where class_id = '${class_id}' AND section_name = '${section_name}' AND status = 1 `;
    let query = con.query(sql, (err, result)=>{
        if(err){
            res.json({msg: 'fail to get class data'+err});
        }
...
router.get('/get-sectionByClassId/:class_id',(req, res) =>{
    let class_id = req.params.class_id;
    let sql = `Select * from tbl_section WHERE class_id = ${class_id} AND status =1 `;
    let query = con.query(sql,(err , result) => {
        if(err){
            res.json({
...
router.put('/update-section/:section_id' , (req , res)=>{
    let section_id = req.params.section_id;
    let sectionUpdateData = {
        section_name:  req.body.section_name,
        teacher_name:  req.body.teacher_name     
    };
    let sql = 'Update tbl_section SET = ? WHERE section_id = ?' ;
    let query = con.query(sql,[sectionUpdateData , section_id ], (err, result)=>{
        if(err){
